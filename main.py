...
def insert_to_bin(cur_dest, cur_pack, i, j, NOW):
    ...
    if cur_pack.is_priority:
        ...
        cur_pack.days_in_center += (cur_pack.st_sent -
                                    cur_pack.first_sending_option)
    else:
        cur_pack.ft_sent = NOW
        cur_pack.days_in_center = (
            cur_pack.ft_sent - cur_pack.first_sending_option)
...
def place_pack(heap, curr_point, cur_dest):
    ...
    if cur_dest.available_bins[curr_point[1]] > 0:
        ...
        print(
            f'placed item size {curr_point[1]} in bin {curr_point[1]} at {cur_dest}')
        ...
        if cur_dest.available_bins[curr_point[1]-1] > 0:
            insert_to_bin(cur_dest, cur_pack,
                          curr_point[0], curr_point[1]-1, NOW)
            print(
                f'placed item size {curr_point[1]} in bin {curr_point[1]-1} at {cur_dest}')
            package_collection_creation(cur_pack, cur_pack.cur_location, NOW)

    elif curr_point[1]-2 >= 1:
        if cur_dest.available_bins[curr_point[1]-2] > 0:
            insert_to_bin(cur_dest, cur_pack,
                          curr_point[0], curr_point[1]-2, NOW)
            print(
                f'placed item size {curr_point[1]} in bin {curr_point[1]-2} at {cur_dest}')
            package_collection_creation(cur_pack, cur_pack.cur_location, NOW)
...
destinations[1].neighbors["big"], destinations[1].neighbors["medium"], destinations[1].neighbors["small"] = [
    2, 3, 4], [4, 2, 3], [4, 2, 3]
destinations[2].neighbors["big"], destinations[2].neighbors["medium"], destinations[2].neighbors["small"] = [
    1, 4], [1, 4], [1, 4]
destinations[3].neighbors["big"], destinations[3].neighbors["medium"], destinations[3].neighbors["small"] = [
    1, 5, 4], [4, 1, 5], [4, 1, 5]
destinations[4].neighbors["big"], destinations[4].neighbors["medium"], destinations[4].neighbors["small"] = [
    1, 5, 2, 6, 3], [6, 2, 5, 1, 3], [6, 2, 5, 1, 3]
destinations[5].neighbors["big"], destinations[5].neighbors["medium"], destinations[5].neighbors["small"] = [
    6, 3, 4], [6, 4, 3], [6, 4, 3]
destinations[6].neighbors["big"], destinations[6].neighbors["medium"], destinations[6].neighbors["small"] = [
    4, 5], [4, 5], [4, 5]
